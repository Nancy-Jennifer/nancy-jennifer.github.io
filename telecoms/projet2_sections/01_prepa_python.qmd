...
::: {.callout-note}
**Exports** — Téléchargements rapides :
- PDF : `01_prepa_python.pdf`
- Markdown (GitHub) : `01_prepa_python.md`
:::

## 1) Contexte & objectifs

Ce document prépare et standardise les données **ARCEP 2024**, **ITU**, **Banque Mondiale** (et dérivés), puis **produit exactement** les fichiers nettoyés suivants (déclarés dans ton notebook) :
...
## 5) Banque Mondiale – Cellular & WDI (sources détectées)
...
# Harmonisation légère (si colonnes usuelles existent)
for col in ("year","annee"):
    if col in wb_cell.columns:
        wb_cell = cast_int(wb_cell, [col])
        break
for col in ("year","annee"):
    if col in wb_wdi.columns:
        wb_wdi = cast_int(wb_wdi, [col])
        break
```
...
if ARCEP_XLSX_GLOBAUX.exists():
    ...
    # Tu avais au moins un export "simplifié" et un "clean" → on persiste les 3 pour stabilité
...
# Deux cas possibles dans ton notebook :
# 1) tu pars d’un WDI "long" / agrégats → on mappe iso3/indicator_code/year/value si présents
# 2) tu lis une source ITU brute séparée
...
# priorité aux fichiers que tu as déjà transformés côté World Bank
candidate_sources = [
    PROC / "03-WorldBank-WDI-long.csv",       # détecté dans ton .ipynb
...
# Schéma de documentation (échantillons)
schemas["itu_clean"] = schema_preview(pd.read_csv(qa_files[7]) if qa_files[7].exists() else pd.DataFrame(), "ITU_clean")
...
- Les blocs **ARCEP** et **ITU Facts & Figures** ont été calés pour **reproduire tes exports** tout en restant **portables** (chemins relatifs, lectures robustes, colonnes normalisées).
