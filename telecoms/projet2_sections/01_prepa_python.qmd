...
# (Les profils colonnes générés sont écrits plus bas dans §9.)
pass
```
...
::: {.callout-tip}
**Pourquoi cette étape ?**  
La Banque Mondiale comporte souvent des variations d’encodage/séparateur.  
On force une lecture **tolérante** et des noms de colonnes **normalisés** avant toute fusion.
:::
...
```{python}
#| label: 1-5-arcep
# Fichiers bruts (mêmes noms que dans le .ipynb)
...
# Voix/Data — habitations & transports (CSV ; sep=';' enc='utf-8')
...
::: {.callout-tip}
**Pourquoi cette étape ?**  
Assurer des **exports strictement nommés** comme dans le .ipynb, mais sans fuite de chemins locaux ni dépendance à un environnement unique.  
Les CSV vides structurés garantissent qu’un **build n’échoue jamais** sur un dépôt public.
:::
...
def count_rows(path: Path) -> int:
    if not path.exists():
        return 0
    try:
        # lignes de données (en enlevant l'entête)
...
# Sauvegarde standardisée (emplacement consistant avec le pipeline)
qa_out = PROC / "p2_qa_prepa.csv"
qa_out.parent.mkdir(parents=True, exist_ok=True)
qa.to_csv(qa_out, index=False)

# Affichage synthétique non-verbeux
display(qa)
```
...
- Les blocs **ARCEP** et **ITU Facts & Figures** reproduisent les exports tout en restant **portables** (chemins relatifs, lectures robustes, colonnes normalisées).  
- En cas d’absence de fichier brut, un CSV structuré vide est généré afin d’éviter toute interruption du processus de rendu ; les fichiers complets sont ensuite régénérés automatiquement lors de la mise à jour des sources.  
- Les noms d’exports sont **strictement identiques** à ceux utilisés dans les scripts de préparation (`.ipynb` ou `.py`).
...
- **9 fichiers nettoyés** et **2 exports ITU Facts & Figures** prêts pour l’EDA R.  
- Journal qualité (QA) automatisé, schéma YAML exporté et lecture tolérante aux variations d’encodage.

::: {.callout-note}
## 🔄 Étape suivante : Analyse exploratoire & Benchmark

Poursuivre vers l’EDA complète :  
➡️ [Accéder à l'exploration des données](02_eda_R.qmd)
:::
