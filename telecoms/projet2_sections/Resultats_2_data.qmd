...
# Installe/charge les packages requis (compatible GitHub Actions)
req <- c("dplyr","tidyr","stringr","forcats","ggplot2","gt","readr","janitor")
...
kpi_core_tbl %>%
  gt::gt() %>%
  gt::tab_header(title = "KPI centraux DATA — médianes (p50)") %>%
  gt::fmt_number(columns = c(dl_med_mbps, ul_med_mbps, lat_med_ms), decimals = 1)
```
...
consistency_tbl <- df %>%
  ...
  dplyr::mutate(dplyr::across(tidyselect::ends_with("_pct"), ~ round(.x, 1))) %>%
  dplyr::arrange(env, dplyr::desc(ge_30_pct))

consistency_tbl %>%
  gt::gt() %>%
  gt::tab_header(title = "Consistency débit descendant — part des tests au-dessus du seuil (%)")
```
...
gap_env %>%
  gt::gt() %>%
  gt::tab_header(title = "Écarts Habitat – Mobilité (p50)") %>%
