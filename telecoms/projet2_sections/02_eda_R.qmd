...
Cette section compare la France à deux références internationales standardisées (Union Européenne et Monde) sur :
- **l’usage d’Internet (% population)**  
- **les abonnements mobile broadband (pour 100 habitants)**  

Les données proviennent des fichiers normalisés ITU *Facts & Figures 2024* contenus dans :  
`data/clean_final/ITU_FactsFigures2024_final_clean.csv`.

Elles permettent de replacer les résultats ARCEP France dans un contexte global cohérent.

---

## Chargement des données ITU
...
On sélectionne l’indicateur exact :  
**indicator == "internet_use_(%)"**

```{r itu-internet-use}
internet_use_df <- itu_final |>
  dplyr::filter(
    indicator == "internet_use_(%)",
    country %in% c("France", "European Union", "World")
  ) |>
...
```{r itu-internet-use-plot, fig.cap="Usage d’Internet (% population) – France vs Europe vs Monde."}
ggplot(internet_use_df,
       aes(x = country, y = value, fill = country)) +
  geom_col(width = 0.6) +
  scale_y_continuous(limits = c(0,100)) +
  labs(
    title    = "Usage d’Internet (% population)",
    subtitle = "Comparaison France – Union Européenne – Monde",
    x        = NULL,
    y        = "Taux d’usage (%)"
  ) +
  guides(fill = "none")
...
- La France se situe **au-dessus de la moyenne mondiale**  
- Elle se place **au niveau des standards européens**, ce qui montre une maturité numérique élevée  
- Ce niveau d’usage contextualise les attentes QoS : **une population très connectée = faible tolérance aux lenteurs**

---

## Abonnements Mobile Broadband (pour 100 habitants)

Indicateur exact dans tes données :  
**indicator == "broadband_subs"**
...
```{r itu-mbb-plot, fig.cap="Mobile Broadband (pour 100 habitants) – France vs Europe vs Monde."}
ggplot(mbb_df,
       aes(x = country, y = value, fill = country)) +
  geom_col(width = 0.6) +
  labs(
    title    = "Mobile Broadband – abonnements pour 100 habitants",
    subtitle = "France comparée à ses deux zones de référence",
    x        = NULL,
...
- La France présente un niveau d’abonnements mobile broadband **aligné avec les pays développés**  
- L’écart avec la moyenne mondiale reste important → **la France se situe dans la zone haute** en termes d’équipement  
- Cela renforce la nécessité d’un **réseau robuste, homogène et performant** sur l’ensemble du territoire.
