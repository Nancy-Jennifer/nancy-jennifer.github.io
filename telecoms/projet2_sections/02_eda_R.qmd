...
```{r}
# Installation auto des packages manquants (CRAN)
...
# Renvoie le premier nom présent parmi 'candidates' ou 'default'
first_present <- function(candidates, names_vec, default) {
...
# remplace l'opérateur %||% (rlang) par une fonction locale
...
voix_kpi <- voix_scored |>
  ...
  dplyr::group_by(operateur, lieu_strate, kpi) |>
  dplyr::summarise(
...
if (nrow(voix_scored) > 0) {
  ...
  voix_flags %>%
    dplyr::filter(flag) %>%
    dplyr::count(lieu_strate, sort = TRUE, name = "nb_alertes") %>%
    dplyr::slice_head(n = 15) %>%
    ggplot2::ggplot(ggplot2::aes(x = reorder(lieu_strate, nb_alertes), y = nb_alertes)) +
    ggplot2::geom_col() + ggplot2::coord_flip() +
