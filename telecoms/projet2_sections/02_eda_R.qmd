...
Cette section s’appuie sur les jeux de données internationaux prétraités dans le dossier `data/clean_final`  
(fichiers `ITU_FactsFigures2024_final_clean.csv` et `ITU_FactsFigures2024_summary_clean.csv`).

Elle permet de répondre directement à la question centrale du projet :  
**« Où se situe la France par rapport à l’international ? »**

---

```{r itu_benchmark_load}
itu_final <- read_csv(
...
itu_2024 <- itu_final |>
  dplyr::filter(year == 2024, !is.na(value))
...
## Usage d’Internet – grandes régions (UIT 2024)

```{r itu_internet_use, fig.cap="Usage d’Internet – grandes régions (UIT 2024)."}
...
if (nrow(internet_region_2024) > 0) {
  ggplot(
    ...
    geom_col() +
    labs(
      title    = "Usage d’Internet – position de l’Europe",
      subtitle = "Comparaison avec les grandes régions du monde (UIT 2024)",
...
- L’Europe fait partie des **régions les plus connectées au monde**.  
- La France, en tant que pays européen, s’inscrit donc dans un **écosystème globalement avancé** en termes de pénétration internet.

---

## Abonnements mobile broadband pour 100 habitants – grandes régions (UIT 2024)

```{r itu_broadband, fig.cap="Abonnements mobile broadband pour 100 habitants – grandes régions (UIT 2024)."}
...
if (nrow(broadband_region_2024) > 0) {
  ggplot(
    ...
    labs(
      ...
      subtitle = "Europe vs autres régions (UIT 2024)",
...
- L’Europe se situe **nettement au-dessus de la moyenne mondiale**.
- Cela indique un parc mobile haut-débit **dense**, cohérent avec les performances Data observées en France.
