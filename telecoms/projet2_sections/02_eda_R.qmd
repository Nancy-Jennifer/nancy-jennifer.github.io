...
L’objectif de cette section est de replacer les performances ARCEP 2024 dans un cadre international, en utilisant des jeux de données UIT prétraités.
...
```{r benchmark-data, message=FALSE, warning=FALSE}
library(tibble)
library(dplyr)
library(ggplot2)

# ------------------------------------------------------------------
# 5.1 Internet use – France vs régions du monde (UIT 2024)
# ------------------------------------------------------------------
# ⚠️ Remplace les valeurs ci-dessous par les vraies valeurs UIT 2024
#    (France, Europe, Monde) que tu as déjà calculées dans ton projet.
itu_internet_use <- tribble(
  ~category,   ~internet_use_pct,
  "France",    92,   # <-- à remplacer par la valeur exacte UIT
  "Europe",    88,   # <-- idem
  "Monde",     70    # <-- idem
) |>
  mutate(
    category = factor(category, levels = c("Monde", "Europe", "France"))
  )

# ------------------------------------------------------------------
# 5.2 Abonnements mobile broadband – France vs régions (UIT 2024)
# ------------------------------------------------------------------
# ⚠️ Même principe : remplace les valeurs par les chiffres UIT 2024.
itu_mbb <- tribble(
  ~category,   ~mbb_100,
  "France",    115,  # <-- valeur UIT pour la France
  "Europe",    120,  # <-- moyenne régionale Europe
  "Monde",     95    # <-- moyenne mondiale
) |>
  mutate(
    category = factor(category, levels = c("Monde", "Europe", "France"))
  )
...
## 5.1 Internet use – France vs régions du monde

```{r itu-internet-use-plot, fig.cap="Usage d’internet – France vs moyennes régionales (UIT 2024)."}
itu_internet_use

ggplot(itu_internet_use,
       aes(x = category, y = internet_use_pct, fill = category)) +
  geom_col(width = 0.6) +
  coord_flip() +
  scale_y_continuous(limits = c(0, 100)) +
  labs(
    title    = "Usage d’internet (% population)",
    subtitle = "Comparaison France vs moyennes régionales – UIT 2024",
    x        = NULL,
    y        = "% de la population"
  ) +
  guides(fill = "none")
...
### 5.1.1 Lecture métier

- La **France** se situe dans le groupe des régions à **très forte pénétration internet**.
- L’**Europe** est globalement au-dessus de la moyenne mondiale, la France restant alignée ou légèrement au-dessus.
- Le **monde** reste en retrait, ce qui met en perspective le niveau de maturité numérique français.

## 5.2 Abonnements mobile broadband – France vs régions

```{r itu-mbb-plot, fig.cap="Abonnements mobile haut-débit mobile – France vs régions mondiales (UIT 2024)."}
itu_mbb

ggplot(itu_mbb,
       aes(x = category, y = mbb_100, fill = category)) +
  geom_col(width = 0.6) +
  coord_flip() +
  labs(
    title    = "Abonnements haut-débit mobile (pour 100 habitants)",
    subtitle = "France vs moyennes régionales – UIT 2024",
    x        = NULL,
...
### 5.2.1 Lecture métier

- La **densité d’abonnements mobile broadband** en France est **très élevée**, cohérente avec une forte intensité d’usage Data.
- L’**Europe** se situe également au-dessus de la moyenne mondiale, ce qui confirme un environnement mobile très connecté.
- Le **monde** reste en-dessous : la France évolue donc dans un contexte de **maturité numérique avancée**.

## 5.3 Synthèse Benchmark

- La France se situe dans un groupe à **forte maturité numérique** selon l’usage d’internet.
- Le niveau d’abonnements mobile broadband montre une **forte intensité d’usage**, cohérente avec les attentes QoS mesurées par l’ARCEP.
- Ce benchmark offre un cadre international permettant de **mettre en perspective** les performances nationales Voix & Data.
...
Recommandations opérationnelles (exemples structurants) :
