...
# Remplace janitor::clean_names() par une version locale simple
clean_names_local <- function(df) {
  new <- names(df)
  new <- tolower(new)
  new <- gsub("[^a-z0-9]+", "_", new)  # remplace espaces, points, etc. par "_"
  new <- gsub("^_|_$", "", new)        # enlève "_" au début/fin
  names(df) <- new
  df
}

# Repertoire de base (data/ est à la racine du repo)
base_dir <- "../.."
...
### 3.1 Construction du KPI Voix (0–100)
...
- Score voix sur **0–100**, pondéré par le nombre de mesures.  
- Permet de dégager un **classement global** des opérateurs sur la fiabilité des appels.

### 3.2 Comparaison par zone et environnement
...
- Les environnements **Transports** sont plus exigeants que les lieux de vie.  
- Certains opérateurs présentent une performance **homogène** entre zones, d’autres ont des **points faibles ciblés**.
...
- < 90 : zones **critiques**, à traiter en priorité (plan d’action court terme).  
...
### 4.2 Performance par zone (si disponible)
...
## 7. Messages clés pour décideurs

- **Voix** : niveau global élevé, mais des **zones à risque** clairement identifiées (par opérateur, zone, environnement).  
- **Data** : contrastes plus marqués entre opérateurs sur la **fluidité de navigation** et les **transports**.  
...
- une base factuelle solide pour la partie **“Résultats & recommandations”** du projet P2 ;
- des **KPI clairs et actionnables** pour les équipes réseau et régulation ;
- un support de communication structuré pour un **recruteur** ou un **décideur non technique**.
