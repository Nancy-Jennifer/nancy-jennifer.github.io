...
## Performance Data par zone (si disponible)
...
Cette section s’appuie sur les jeux de données internationaux prétraités dans le dossier `data/clean_final` (fichiers `ITU_FactsFigures2024_final_clean.csv` et `ITU_FactsFigures2024_summary_clean.csv`). Ils permettent de replacer les résultats ARCEP dans un cadre plus large.

Les indicateurs disponibles couvrent notamment :

- l’**usage d’internet** dans la population ;
- les **abonnements mobile broadband** (par 100 habitants) ;
- des indicateurs de **couverture** et de **capacité réseau** (ex. couverture 4G/5G, débits).

Dans le projet complet, ces données sont exploitées dans un fichier dédié pour :

- comparer la France à différentes zones de référence (monde, régions, groupes de pays) ;
- illustrer si la France se situe plutôt dans un groupe de **leaders**, de **pays dans la moyenne** ou de **pays en retard** selon les indicateurs ;
- relier ces écarts macroscopiques aux constats issus des mesures terrain ARCEP.

Ici, on garde la section volontairement synthétique pour ne pas alourdir la lecture : le message clé pour un recruteur est que tu sais **brancher une EDA nationale sur des sources internationales normalisées** et construire un **benchmark structuré** autour de tes KPI Voix / Data.
...
Pour un recruteur ou un décideur réseau, ce rapport démontre que je suis capable de :

- construire des **KPI robustes** (Voix 0–100, Data < 10 s, débits médians) ;
- identifier des **zones à risque** et les prioriser dans un plan d’action ;
- analyser les écarts de performance entre opérateurs et environnements (habitations vs transports, zones denses vs rurales, etc.) ;
- proposer une lecture orientée **qualité d’expérience client** et **priorisation opérationnelle** ;
- articuler ces constats avec un **benchmark international** fondé sur les données UIT / Banque mondiale.
