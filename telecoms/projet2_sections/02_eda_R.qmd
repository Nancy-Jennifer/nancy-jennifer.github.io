...
# Helper pour nettoyer les noms de colonnes sans dépendre de janitor
...
# Racine du repo (02_eda_R.qmd est dans telecoms/projet2_sections)
base_dir <- "../.."

operateurs_levels <- c("Orange", "SFR", "Bouygues", "Free")

# Standardisation des opérateurs sans stringr
...
# Petites fonctions de formatage sans scales
label_num1 <- function(x) sprintf("%.1f", x)
label_pct  <- function(x) sprintf("%d%%", round(100 * x))
```
...
```{r itu-harmonisation}
# Harmonisation robuste : on garantit la présence de indicator, country, year, value
...
if (!"indicator" %in% names(itu_summary)) {
  if ("indicator_code" %in% names(itu_summary)) {
    itu_summary$indicator <- itu_summary$indicator_code
  }
}
...
ggplot2::ggplot(
  ...
  ggplot2::scale_y_continuous(labels = label_num1) +
  ggplot2::labs(
...
- Priorisation des **plans d’amélioration réseau** par opérateur et type de zone.
...
- En dessous de **95**, l’expérience client devient sensible ; en dessous de **90**, les zones sont critiques.
- Liste courte des combinaisons opérateur × zone × contexte à traiter en priorité dans un **plan d’action 90 jours**.
...
- Vision directe du **confort de navigation web** par opérateur.
- Mise en évidence des **écarts de QoE Data** entre lieux de vie et transports.
- Base pour des objectifs **contractuels** (ex. ≥ 95 % de pages < 10 s).
...
- Identification des **zones Data en sous-performance** à la maille opérateur.
- Priorisation des **investissements radio** dans les zones à fort trafic mais QoE dégradée.
...
Les 10 combinaisons les plus faibles constituent une **liste courte** pour les équipes techniques : volume significatif et **risque d’insatisfaction client** élevé.
...
- Positionne la **France** par rapport aux grandes régions (Europe, Amériques, etc.).
- Vérifie la cohérence entre **niveau d’usage** et **niveau de QoS**.
...
```{r mobile-broadband-plot, eval = nrow(mobile_broadband_strict) > 0, fig.cap="Abonnements mobile broadband pour 100 habitants – classement mondial."}
ggplot2::ggplot(
...
- Support pour un discours de régulation : cohérence entre **niveau d’équipement** et **niveau de QoS**.
...
- Mise en évidence des axes où la France est **forte**, **alignée** ou **en retrait**.
...
- Utile pour un discours de régulation : à **PIB comparable**, la France est en avance / dans la moyenne / en retard sur les débits.
