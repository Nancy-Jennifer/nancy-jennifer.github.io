...
# Coalescence et sélection robuste de colonnes
`%||%` <- function(a, b) {
...
if (nrow(arcep_clean) > 0) {
  ...
  df <- arcep_clean |>
    dplyr::mutate(dplyr::across(dplyselect::any_of(c("annee","year")), as.integer)) |>
...
# Pivot/standardisation Voix (si colonnes opérateurs étalées)
bind_voix <- function(df){
  ...
  df |>
    ...
    dplyr::mutate(
      ...
      base_val = suppressWarnings(as.numeric(dplyr::coalesce(base_val, 1)))
    ) |>
...
# Choix dynamique de la localisation (si absente, agrégation nationale)
...
# Classement opérateurs – réussite moyenne (si dispo)
if (nrow(voix_kpi) > 0) {
  df_rank <- voix_kpi |>
...
# Zones à risque (ex. réussite < 95% ou échec > 2%), si colonnes disponibles
...
- Pour une **lecture métier**, construire des **KPI composites** par axe (Voix disponibilité, Data performance, Radio qualité), avec seuils cibles.
